...
public class LogMonitorLauncher extends CoroutineDelegateImpl implements MonitorLauncher {
    ...
    public void startSuccess(MonitorEntity monitorEntity) {
        boolean logPrint = properties.getBoolean(CoroutineConstant.MONITOR_LOG_SUCCESS_PRINT);
        if (logPrint) {
            ...
            if (referenceType == ReferenceType.COMPONENT_REFERENCE) {
                ...
                String keyName = methodEntity.getKeyName();
                String key = methodEntity.getKey();
                String method = methodEntity.getMethod();
                String parameterTypes = methodEntity.getParameterTypes();

                LOG.info("{} completed, referenceType={}, index={}, categoryName={}, ruleName={}, chainName={}, " + keyName + "={}, method={}, parameterTypes={}, returnType={}, spentTime={} ms, id={}", StringUtil.firstLetterToUpper(executorType.toString()), referenceType.toString(), index, id.getCategoryName(), id.getRuleName(), chainName, key, method, parameterTypes, returnType, endTime - startTime, id.getId());
    ...
    public void startFailure(MonitorEntity monitorEntity) {
        boolean logPrint = properties.getBoolean(CoroutineConstant.MONITOR_LOG_FAILURE_PRINT);
        if (logPrint) {
            ExecutorType executorType = monitorEntity.getExecutorType();
            CoroutineId id = monitorEntity.getId();
            ReferenceEntity referenceEntity = monitorEntity.getReferenceEntity();
            ...
            if (referenceType == ReferenceType.COMPONENT_REFERENCE) {
                ...
                LOG.error("{} failed, referenceType={}, index={}, chainName={}, " + keyName + "={}, method={}, parameterTypes={}, spentTime={} ms, id={}", StringUtil.firstLetterToUpper(executorType.toString()), referenceType.toString(), index, chainName, key, method, parameterTypes, endTime - startTime, id.getId(), exception);
