...
import com.nepxion.coroutine.serialization.SerializerException;
import com.nepxion.coroutine.serialization.SerializerLoader;
...
public class ZookeeperInvoker {
    ...
    public <T> T getObject(byte[] data, Class<T> clazz) throws SerializerException {
        try {
            return SerializerLoader.load().deserialize(data, clazz);
        } catch (Exception e) {
    ...
    public byte[] getData(Serializable object) throws SerializerException {
        try {
            return SerializerLoader.load().serialize(object);
        } catch (Exception e) {
            throw new SerializerException("Class can't be compatible", e);
        }
    }
